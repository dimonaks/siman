...
#             if hasattr(curset, 'magnetic_moments') and curset.magnetic_moments and ('ISPIN' in curset.vasp_params.keys()) and curset.vasp_params['ISPIN'] == 2:
    ...
    def dos(self, isym=None, el=None, i_at=None, iatoms=None, multi=None, res_show='fomag', *args, **kwargs):
        ...
        if fontsize:
            ...
            header.mpl.rc('font', size=SMALL_SIZE)
            # fontsize of the axes title
            header.mpl.rc('axes', titlesize=SMALL_SIZE)
            # fontsize of the x and y labels
            header.mpl.rc('axes', labelsize=MEDIUM_SIZE)
            # fontsize of the tick labels
            header.mpl.rc('xtick', labelsize=SMALL_SIZE)
            # fontsize of the tick labels
            header.mpl.rc('ytick', labelsize=SMALL_SIZE)
            header.mpl.rc('legend', fontsize=SMALL_SIZE)    # legend fontsize
            # fontsize of the figure title
            header.mpl.rc('figure', titlesize=BIGGER_SIZE)
    ...
    def get_file(self, filetype='', nametype='', up='up1', root=0):
        ...
        if os.path.exists(path_to_file) and '2' not in up:
            ...
            if out:
                printlog('File', path2file_cluster,
                         'was not found, trying archive:', header.PATH2ARCHIVE, imp='Y')
                # printlog(out, imp  = 'Y')
        ...
        if out:
            if header.PATH2ARCHIVE:
                ...
                if out:
                    printlog('File', path_to_file,
                             ' was downloaded from archive', imp='e')

                else:
                    printlog('File', path_to_file_scratch,
                             'was not found, trying local archive path', imp='Y')

            else:
                printlog(
                    'project_conf.PATH2ARCHIVE is empty, trying local archive path', imp='Y')
        ...
        if out:
            if header.PATH2ARCHIVE_LOCAL:
                ...
                if os.path.exists(path_to_file_AL):
                    ...
                    printlog('File', path_to_file_AL,
                             'not found in local archive', imp='Y')
