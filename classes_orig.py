...
from picture_functions import fit_and_plot as my_plot
...
sys.path.append('/home/dim/Simulation_wrapper/ase') #path to siman1 library
...
class CalculationVasp(Calculation):
    ...
    def make_incar_and_copy_all(self, update):
        ...
        try:
            self.set.vasp_params['MAGMOM']
            if "*" not in self.set.vasp_params['MAGMOM']:
                self.set.vasp_params['MAGMOM'] = str(self.init.natom) +"*"+ self.set.vasp_params['MAGMOM']
        except:
            pass
    ...
    if "up" in update:
        ...
        for input_geofile in geofilelist:
            ...
            if curv == id[2]:
                ...
                if input_geo_format == 'abinit':
                    # print input_geofile
                    # print calc[id].dir+input_geofile.split['/'][-1]
...
def add_loop(it,ise,verlist, calc = None, conv = None, varset = None, up = 'test', typconv="", from_geoise = '', inherit_option = None, 
    coord = 'direct', savefile = "allx", show = [], comment = None, input_geo_format = 'abinit', ifolder = None,):
...
def res_loop(it, setlist, verlist,  calc = None, conv = {}, varset = {}, analys_type = 'no', b_id = (), 
    ...
    comment = None, input_geo_format = None, savefile = None, energy_ref = 0, ifolder = None ):
    """Read results
    INPUT:
    ...
    for inputset in setlist:
        for v in verlist:
            ...
            if analys_type in ('e_seg', 'coseg'):
                try:
                    b_id[1] 
                    b_id = (b_id[0], b_id[1], id[2])
                except:
                    b_id = (b_id[0], id[1], id[2])
            ...
            if   b_id :
                ...
                if 'gbe' in analys_type:      
                    ...
                    outst2 += " {:.0f} & {:.2f} ".format( (diffE - energy_ref)*1000, (v - v_b) ).center(6)
                    outst2 +='&'
