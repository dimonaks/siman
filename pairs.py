...
from functions import image_distance, replic, write_xyz,xcart2xred, xred2xcart, local_surrounding, element_name_inv
from impurity import insert, add_impurity
...
def find_pairs(base_name, segtyp, in_calc, central_atoms = [], xcart1imp = None, input_dlist_coseg = None, prec = 2, gvolume_config_num = None,
    ...
    def write_geometry_files(dlist, in_calc, xcart_pores, segtyp, take_final_rprimd_from = None, add_name_before = '', tlist = [], configver = False,
        ...
        for i, el in enumerate( tlist): #by all found structures
            ...
            if    not hasattr(calc.build, 'listadded') or calc.build.listadded == [None]: calc.build.listadded = range(stn.natom - 2, stn.natom) #list of atoms which were added
            else: calc.build.listadded.extend(                           range(stn.natom - 2, stn.natom) )
